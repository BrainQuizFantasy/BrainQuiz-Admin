    ...
    public function get_subcategory_by_maincategory_post()
    {
        ...
        if ($this->AccessKey != $this->post('access_key')) {
            ...
            if ($this->post('category') && $user_id) {
                ...
                $this->db->select('s.*, ' . $no_of_que . ' (select max(`level` + 0) from tbl_question q where q.subcategory=s.id ) as maxlevel, (SELECT count(*) from tbl_user_subcategory us WHERE us.subcategory_id = s.id and us.user_id = '.$user_id.' ) as has_unlocked');
    ...
    public function get_categories_post()
    {
        ...
        if ($this->AccessKey != $this->post('access_key')) {
            ...
            if ($this->post('type') && $user_id) {
                ...
                $this->db->select('c.*,(SELECT @no_of_subcategories := count(`id`) from tbl_subcategory s WHERE s.maincat_id = c.id and s.status = 1 ) as no_of, ' . $no_of_que . ' if(@no_of_subcategories = 0, (SELECT @maxlevel := MAX(`level`+0) from tbl_question q WHERE c.id = q.category ),@maxlevel := 0) as maxlevel, (SELECT count(*) from tbl_user_category uc WHERE uc.category_id = c.id and uc.user_id = '.$user_id.' ) as has_unlocked');
                $this->db->where('type', $type);
                if ($this->post('id')) {
                    $id = $this->post('id');
                    $this->db->where('id', $id);
    ...
    public function get_system_configurations_post()
    {
        if ($this->AccessKey != $this->post('access_key')) {
            ...
            $setting = [
                ...
                'payment_mode', 'payment_message', 'per_coin', 'coin_amount' ,'currency_symbol', 'coin_limit', 'difference_hours', 'app_maintenance', 'maximum_winning_coins', 'maximum_coins_winning_percentage', 'score',
